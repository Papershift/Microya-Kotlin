...
abstract class Endpoint {
    ...
    /** The query parameters to send for a specific endpoint (part after ? in URLs, e.g. google.com?query=Harry+Potter). */
    abstract val queryParameters: Map<String, String>

    /**
     *  The mocked response for testing purposes. Will be returned instead of making actual calls when [ApiProvider] [mockingBehavior] is set.
     */
     ...
     * @return is the formed request containing the base url, header and body to be sent to the server.
     */
     ...
     * This serializer key is generated by kotlinx serialisation library automatically when using a polymorphic [SerializersModule]
     ...
     */
    fun mockJsonString(statusCode: Int, bodyJson: String, headers: Map<String, String> = emptyMap()): MockedResponse =
        MockedResponse(subpath, statusCode, bodyJson, headers)

    /**
     * Creates a [MockedResponse] object with the given status, body [Serializable] object and headers (optional).
     */
    inline fun <reified T> mockResponseObject(statusCode: Int, bodySerializable: T, headers: Map<String, String> = emptyMap()): MockedResponse =
