...
sealed class JsonApiException : Exception() {
    /** The request was sent, but the server response was not received. Typically an issue with the
     *  internet connection.
     */
    object NoResponseReceived : JsonApiException()

    /** The request was sent and the server responded, but the response did not include any body
     *  although a body was requested.
     */
    object NoDataInResponse : JsonApiException()

    /** The request was sent and the server responded with a body, but the conversion of the body
     * to the given type failed.
     */
    data class ResponseDataConversionFailed(
        val type: String,
        val exception: SerializationException
    ) : JsonApiException()

    /** The request was sent and the server responded, but the server reports that something is
    ...
    /** The request was sent and the server responded, but there seems to be an error which needs
     *  to be fixed on the server.
     */
    ...
    /** An unexpected exception was thrown. This should probably be fixed in the `JsonApi` class
     * implementation itself.
     */
    data class UnexpectedException(val exception: Exception) : JsonApiException()

    /**
     *  The [mockingBehavior] was set to non-nil (for testing) but no `mockedResponse` was provided
